...
{
    if (FilesClass::demoStudentsMap.find(id) != FilesClass::demoStudentsMap.end()) {
        Student* student = &FilesClass::demoStudentsMap[id];
        vector<string> courses = student->getCourses();
        cout << "\nthe list returned succ\n";
        QMessageBox::information(this, "Success", "admingetcourses sucss");
        return courses;
    }
    else {
...
void UploadEditStudentGradesPage::loadStudentCourses(string studentId) {
    ...
    for (string& course : courseList) {
        ui->comboBoxCourse->addItem(QString::fromStdString(course));
...
void UploadEditStudentGradesPage::onShowCoursesClicked() {
    QMessageBox::information(this, "Success", "onshowwwwwwwwwwwww sucss");
